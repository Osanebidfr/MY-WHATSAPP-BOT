...
// WELCOME HANDLER — clean, optimized, no more (object Object)
sock.ev.on("group-participants.update", async (update) => {
  try {
    ...
    for (const p of participants) {
      const userJid = normalizeJid(p);

      // Get contact info safely
      const saved = store.contacts?.[userJid] || {};

      // Prevent (object Object) — only accept strings
      const displayName =
        (typeof saved.name === "string" && saved.name) ||
        (typeof saved.notify === "string" && saved.notify) ||
        userJid.split("@")[0];
      ...
      try {
        ...
        ctx.fillText(
          `Generated by ${config.botName || "Bot"} • ${new Date().toLocaleString("en-GB", {
        ...
        await safeSend(groupJid, {
          image: buffer,
          caption: `👋 Welcome @${userJid.split("@")[0]}!`,
          mentions: [userJid],
        });
      ...
      // If image fails, send text welcome
      if (!sent) {
        await safeSend(groupJid, {
