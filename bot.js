...
// welcome handler: when participants join, send a welcome card (image if canvas available)
sock.ev.on("group-participants.update", async (update) => {
  try {
    const { id: groupJid, participants, action } = update;
    if (!participants || !participants.length) return;
    if (action !== "add") return; // only welcomes on add

    for (const p of participants) {
      const newJid = normalizeJid(p);
      const display =
        (store.contacts && (store.contacts[newJid] || store.contacts[p])) || {};
      const displayName =
        display.name || display.notify || newJid.split("@")[0];
      ...
      try {
        ...
        ctx.fillText("We’re happy to have you here 😊", 60, 260);
        ...
        ctx.fillText(
          `Generated by ${config.botName || "Our Bot"} • ${new Date().toLocaleString("en-GB", {
        ...
        // Generate Buffer and save file (for debugging/logs)
        const buffer = canvas.toBuffer("image/png");
        const outPath = path.join(DATA_DIR, `welcome_${Date.now()}.png`);
        fs.writeFileSync(outPath, buffer);

        // Send directly from Buffer (avoids ENOENT)
        await safeSend(groupJid, {
          image: buffer,
          caption: `👋 Welcome ${displayName} to the group!`,
        });

        sent = true;
      } catch (err) {
        console.error("Error generating welcome image:", err.message || err);
        sent = false;
      }

      if (!sent) {
        await safeSend(groupJid, {
          text: `🎉 Welcome ${displayName} to the group!`,
        });
      }
    }
  } catch (e) {
    console.error("group-participants.update handler error:", e?.message ?? e);
  }
});
...
// TAG / TAGALL - safe, non-blocking, mentions everyone but shows only an emoji
if ((cmd === "tag" || cmd === "tagall") && isGroup) {
  try {
    ...
    if (participants.length <= CHUNK_SIZE) {
      await sock.sendMessage(remoteJid, { text: tagEmoji, mentions: participants });
    } else {
      for (let i = 0; i < participants.length; i += CHUNK_SIZE) {
        const chunk = participants.slice(i, i + CHUNK_SIZE);
        await sock.sendMessage(remoteJid, { text: `${tagEmoji} (${Math.floor(i / CHUNK_SIZE) + 1})`, mentions: chunk });
  ...
  return;
      ...
      if (cmd === "gpt") {
        ...
        try {
          ...
          if (!key) { await safeSend(remoteJid, { text: "⚠️ Missing OpenAI API key. Set OPENAI_API_KEY in .env or config.openaiApiKey in config.json." }); return; }
